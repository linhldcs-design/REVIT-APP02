...
- Đọc Grid line, rectangle cột, polyline kín và hình học 2D trong block/nested block theo chế độ read-only; dùng điểm móc nguồn AutoCAD và điểm móc đích Revit cho cùng phép biến đổi.
- Tab Column có bảng chọn candidate, cấu hình family/kích thước/level/offset/rotation và preview 2D/3D trước khi tạo. Toolbar 2D được đặt trên một hàng riêng để không đè canvas; preview 3D hỗ trợ lăn chuột để zoom và giữ-kéo chuột trái để orbit, kể cả khi bắt đầu kéo trên vùng nền trống.
