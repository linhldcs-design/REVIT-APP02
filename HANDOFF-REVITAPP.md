...
- Release mới nhất đã phát hành: `v1.9.1`
...
- `v1.9.1`: lệnh `Xuất DWG Model` nhận mọi bản AutoCAD đã đăng ký Automation, không riêng AutoCAD 2024. Trước đó code chỉ hỏi ProgID `AutoCAD.Application.24.3`, nên máy chỉ cài AutoCAD 2027 báo "Không tìm thấy AutoCAD đầy đủ trên máy" và khoá nút Xuất DWG. Nay dò lần lượt `AutoCAD.Application.26` (2027), `.25.1` (2026), `.25` (2025), `.24.3` (2024) rồi ProgID không kèm phiên bản, lấy bản mới nhất — cùng cách `AutoCadSelectionService` và `AutoCadModelSelectionService` vẫn dùng; thông báo lỗi in ProgID thực tế thay vì luôn ghi 2024. Phát hành tách riêng từ tag `v1.9.0`, không kèm nhánh `Create Slab` đang phát triển. Test 390/390 đạt; build R22–R27 thành công. Đã xác nhận trên máy có AutoCAD 2027 (`R26.0`, `acad.exe R26.0.60.0.0`): `IsAvailable()` trả `true` và resolve đúng `AutoCAD.Application.26`.
- Lưu ý cho phiên sau: `v1.9.1` mới kiểm chứng bước dò ProgID; chưa chạy lại runtime smoke xuất DWG đầy đủ trên AutoCAD 2027. Cần export thử một Print Set thật để xác nhận EXPORTLAYOUT, DIMLFAC và Text Style vẫn đúng trên bản 2027.
- Cảnh báo đã biết: một số máy còn ProgID mồ côi sau khi gỡ AutoCAD (ví dụ `AutoCAD.Application.25` vẫn đăng ký dù thư mục cài đã hết `acad.exe`). Việc dò ProgID không kiểm tra `LocalServer32` có trỏ tới file thật; nếu bản mồ côi lại đứng trước trong danh sách ưu tiên thì `CreateApplication()` sẽ lỗi lúc khởi động thay vì bị chặn sớm.
